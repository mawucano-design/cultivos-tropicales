...
def extraer_kml_de_kmz(kmz_path, tmpdir):
    ...
    try:
        with zipfile.ZipFile(kmz_path, 'r') as kmz_file:
            ...
            # Extraer el primer KML encontrado
            kml_file = kml_files[0]
            kmz_file.extract(kml_file, tmpdir)
            
            return os.path.join(tmpdir, kml_file)
    except Exception as e:
...
def procesar_kml_avanzado(kml_path):
    ...
    try:
        ...
        st.error(f"Error al procesar KML: {str(e)}")
        return None
...
def procesar_archivo_carga_mejorado(uploaded_file):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if file_ext == 'kmz':
                # Procesar KMZ
                st.info("📦 Procesando archivo KMZ...")
                ...
                # Procesar GeoJSON
                st.info("📋 Procesando GeoJSON...")
                gdf = gpd.read_file(file_path)
                
                if gdf.empty:
                    st.error("El GeoJSON no contiene geometrías válidas")
                    return None
                ...
                return procesar_archivo_carga(uploaded_file)
            ...
            if geometry is None or geometry.is_empty:
                st.error("No se pudo extraer una geometría válida del archivo")
                return None
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        # Usar la función mejorada para procesar
        geometry = procesar_archivo_carga_mejorado(uploaded_file)
        if geometry is not None:
